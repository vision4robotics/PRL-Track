...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params += [{"params": model.grader.parameters(), "lr": cfg.TRAIN.BASE_LR}]
...
def main():
    ...
    if rank == 0:
        ...
        if cfg.TRAIN.LOG_DIR:
            add_file_handler(
                "global", os.path.join(cfg.TRAIN.LOG_DIR, "logs.txt"), logging.INFO
            )
    ...
    if cfg.BACKBONE.PRETRAINED:
        ...
        backbone_path = os.path.join(
            cur_path, "../pretrained_models/", cfg.BACKBONE.PRETRAINED
        )
    ...
    if cfg.TRAIN.RESUME:
        ...
        assert os.path.isfile(cfg.TRAIN.RESUME), "{} is not a valid file.".format(
            cfg.TRAIN.RESUME
        )
        model, _, cfg.TRAIN.START_EPOCH = restore_from(
            model, optimizer, cfg.TRAIN.RESUME
        )
