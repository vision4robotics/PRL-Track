...
def ca():
    ...
    name_list = os.listdir(path + "/anno_l")
    ...
    for jj in range(a):
        ...
        txt = path + "/anno_l/" + str(name_list[jj]) + ".txt"
        bbox = []
        f = open(txt)  # 返回一个文件对象
        file = f.readlines()
        li = os.listdir(imgs)
...
class UAV112LDataset(Dataset):
    def __init__(self, name, dataset_root, load_img=False):
        super(UAV112LDataset, self).__init__(name, dataset_root)
        meta_data = ca()
        ...
        for video in pbar:
            ...
            self.videos[video] = UAVVideo(
                video,
                dataset_root + "/data_seq",  # 图片对应的路径
