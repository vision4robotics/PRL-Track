...
class LogScheduler(LRScheduler):
    def __init__(
        self, optimizer, start_lr=0.03, end_lr=5e-4, epochs=50, last_epoch=-1, **kwargs
    ):
        ...
        **kwargs
        ...
        if end_lr is not None:
            if start_lr is None:
                ...
                mult = math.pow(end_lr / start_lr, 1.0 / (epochs // step))
        self.start_lr = start_lr
        ...
        **kwargs
        ...
        if end_lr is not None:
            if start_lr is None:
                ...
                mult = math.pow(end_lr / start_lr, 1.0 / len(steps))
        self.start_lr = start_lr
...
class LinearStepScheduler(LRScheduler):
    def __init__(
        self, optimizer, start_lr=0.01, end_lr=0.005, epochs=50, last_epoch=-1, **kwargs
    ):
        self.start_lr = start_lr
        self.end_lr = end_lr
...
def _build_lr_scheduler(optimizer, config, epochs=50, last_epoch=-1):
    return LRs[config.TYPE](
        optimizer, last_epoch=last_epoch, epochs=epochs, **config.KWARGS
    )
...
def _build_warm_up_scheduler(optimizer, epochs=50, last_epoch=-1):
    ...
    sc2 = _build_lr_scheduler(
        optimizer, cfg.TRAIN.LR, epochs - warmup_epoch, last_epoch
    )
...
if __name__ == "__main__":
    ...
    log = {"type": "multi-step", "start_lr": 0.01, "mult": 0.1, "steps": [10, 15, 20]}
    lr = build_lr_scheduler(optimizer, log)
    print(lr)
