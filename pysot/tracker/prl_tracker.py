...
class PRLTrack(SiameseTracker):
    def __init__(self, model):
        super(PRLTrack, self).__init__()
    ...
    def generate_anchor(self, mapp):
        ...
        x = np.tile(
            (cfg.ANCHOR.STRIDE * (np.linspace(0, size - 1, size)) + 63)
            - cfg.TRAIN.SEARCH_SIZE // 2,
            size,
        ).reshape(-1)
        y = np.tile(
            (cfg.ANCHOR.STRIDE * (np.linspace(0, size - 1, size)) + 63).reshape(-1, 1)
            - cfg.TRAIN.SEARCH_SIZE // 2,
            size,
        ).reshape(-1)
    ...
    def init(self, img, bbox):
        ...
        z_crop = self.get_subwindow(
            img, self.center_pos, cfg.TRACK.EXEMPLAR_SIZE, s_z, self.channel_average
        )
